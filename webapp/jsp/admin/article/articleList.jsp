...
function f_initGrid(){
    g = manager = $("#maingrid").ligerGrid({
		...
		{ display: '栏目', name: 'columnCode',type: 'text' , width: '15%',render: function (rowdata, rowindex, value)
        {
        	var r = "-";
        	<c:forEach items="${articleColumnList}" var="articleColumn">
        	if(rowdata.columnCode == '${articleColumn.code}'){
        		r = "${articleColumn.name}";
        	}
        	</c:forEach>
		...
		{ display: '类别', name: 'categoryCode',type: 'text' , width: '15%',render: function (rowdata, rowindex, value)
        {
        	var r = "-";
        	<c:forEach items="${articleCategoryList}" var="articleCategory">
        	if(rowdata.categoryCode == '${articleCategory.code}'){
        		r = "${articleCategory.name}";
        	}
        	</c:forEach>
		...
		{ display: '作者', name: 'auth',type: 'text' , width: '15%'},
		{ display: '短标题', name: 'shortTitle',type: 'text' , width: '15%'},
		{ display: '导读图', name: 'articleImg',type: 'text' , width: '15%'},
		//{ display: '摘要', name: 'brief',type: 'text' , width: '15%'},
		//{ display: '内容', name: 'content',type: 'text' , width: '15%'},
		{ display: '文章状态', name: 'status',type: 'text' , width: '15%'},
...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/article/articleAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//article/articleAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/article/articleAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/article/articleAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		var row = manager.getSelectedRow();
        if (!row) {$.ligerDialog.warn('请选择行'); return; }
		$.ligerDialog.confirm('确认删除模块 ' + row.columnCode + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/article/articleAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="articleForm" id="articleForm" method="post" action="articleAction.${actionExt}" >
...
<div id="searchbar">
栏目编码：<input id="article.columnCode" type="text" name="article.columnCode" value="${article.columnCode }"/>
