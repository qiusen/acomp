...
function f_initGrid(){
    g = manager = $("#maingrid").ligerGrid({
		...
		{ display: '栏目', name: 'columnId',type: 'text' , width: '15%',render: function (rowdata, rowindex, value)
        {
        	var r = "-";
        	<c:forEach items="${articleColumnList}" var="articleColumn">
        	if(rowdata.columnId == ${articleColumn.id}){
...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/articleCategory/articleCategoryAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//articleCategory/articleCategoryAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/articleCategory/articleCategoryAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/articleCategory/articleCategoryAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除模块 ' + row.code + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/articleCategory/articleCategoryAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="articleCategoryForm" id="articleCategoryForm" method="post" action="articleCategoryAction.${actionExt}" >
...
<div id="searchbar">
编码：<input id="articleCategory.code" type="text" name="articleCategory.code" value="${articleCategory.code }"/>
